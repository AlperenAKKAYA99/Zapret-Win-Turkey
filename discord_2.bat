@echo off
chcp 65001 >nul  :: UTF-8 karakter kodlamasını etkinleştirir.

:: Komut dosyasının bulunduğu dizine geçiş yap
cd /d "%~dp0"
...
:: Bin dizinini değişken olarak ayarla
set BIN=%~dp0bin\

:: Zapret servisini başlat (genel ayarlar)
start "zapret: general" /min "%BIN%winws.exe" ^
  ...
  --filter-tcp=443 --hostlist="list-general.txt" --dpi-desync=split2 ^
